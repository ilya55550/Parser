...
async def get_product(session, link_product):
    async with session.get(link_product, headers=headers) as response:
        bsoup = BeautifulSoup(await response.text(), 'lxml')
        ...
        return title, price, description, image_url


async def parse_card_product():
    async with aiohttp.ClientSession() as session:
        task_list = []
        for link_product in generator_link_products():
            task_list.append(asyncio.ensure_future(get_product(session, link_product)))

        products = await asyncio.gather(*task_list)
        return products


def main():
    start_time = time.time()
    res = asyncio.run(parse_card_product())
